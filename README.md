...
The package includes ERA-Interim fields for validation. The ERA-Interim data can be downloaded from here. The data is provided on the native grid (`latitude=256, longitude=512`), and the package will interpolate the forecast data to the ERA-Interim grid. For precipitation, [Integrated Multi-satellitE Retrivers for GPM](https://gpm.nasa.gov/data/imerg) is the default validation dataset. IMERG covers 2000-2023 and is interpolated to (`latitude=241`,`longitude=480`). Th package will interpolate the forecast data to the the IMERG grid. 
